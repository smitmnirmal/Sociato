...
public class MyConnection extends Fragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        mProgress.setTitle("Loading Connections...");
        mProgress.setMessage("Please wait...");
        mProgress.setCancelable(false);
        mProgress.setIndeterminate(true);

        recyclerView=(RecyclerView)getActivity().findViewById(R.id.rv_connection);
        ...
        if(count<1){
            ...
            Snackbar.make(view,"Sorry you have no connection!",Snackbar.LENGTH_LONG).setAction("Action", null).show();
        }
                                                ...
                                                try {
                                                    ...
                                                    if(myNetwork.toString().contains(userid)){
                                                        if(!avatarurl.equals("null")){
                                                            connection connection= new connection(avatarurl,username,"Remove Connection",userid,"none");
                                                            connectionList.add(connection);
                                                        }
                                                        else {
                                                            connection connection= new connection("none",username,"Remove Connection",userid,"none");
                                                            connectionList.add(connection);
                                                        }
                                                        connectionAdapter.notifyDataSetChanged();
                                                        continue;
